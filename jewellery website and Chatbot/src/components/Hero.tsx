...
export function Hero({ onOpenChat }: { onOpenChat: () => void }) {
  return (
    <section id="home" className="relative min-h-[85svh] lg:min-h-[88vh] w-full overflow-hidden">
      ...
      {/* Floating Sparkles & Light Particles - Hidden on mobile for GPU performance optimization */}
      <div className="hidden lg:block absolute inset-0 overflow-hidden pointer-events-none z-0">
        <div className="absolute top-[20%] left-[15%] h-56 w-56 rounded-full bg-gold/5 blur-[70px] animate-float-slow" />
        <div className="absolute bottom-[25%] right-[10%] h-72 w-72 rounded-full bg-gold/5 blur-[90px] animate-float-medium" />
        <div className="absolute top-[35%] right-[25%] h-36 w-36 rounded-full bg-gold/5 blur-[50px] animate-float-fast" />
      </div>

      <div className="relative z-10 mx-auto flex min-h-[85svh] lg:min-h-[88vh] max-w-7xl flex-col items-center justify-center px-6 pt-24 pb-16 md:pb-44 text-center">
        <div className="animate-fade-in mb-6 inline-flex items-center gap-2 rounded-full border border-gold/30 bg-white/5 px-4 py-1.5 text-[9px] md:text-[10px] uppercase tracking-[0.3em] text-gold backdrop-blur-md">
          <Sparkles className="h-3 w-3 text-gold/80" /> Est. 2003 · Palakonda
        </div>
        ...
        >
          ...
          >
            <Sparkles className="h-3.5 w-3.5 transition-transform group-hover:rotate-12" /> Chat
            With Expert
          </button>
        </div>
        ...
        >
          <span className="flex items-center gap-2">
            <span className="text-gold text-xs">✔</span> BIS Hallmarked
          </span>
          <span className="hidden sm:inline h-3 w-px bg-white/15" />
          <span className="flex items-center gap-2">
            <span className="text-gold text-xs">✔</span> Certified Diamonds
          </span>
          <span className="hidden sm:inline h-3 w-px bg-white/15" />
          <span className="flex items-center gap-2">
            <span className="text-gold text-xs">✔</span> Since 2003
          </span>
        </div>

        <div className="animate-fade-in absolute bottom-20 left-1/2 hidden -translate-x-1/2 flex-col items-center gap-2 text-white/85 md:flex">
          <span className="text-[9px] uppercase tracking-[0.35em] font-medium">Scroll</span>
          <div className="h-10 w-px bg-gradient-to-b from-white/80 to-transparent animate-shimmer" />
        </div>
      </div>
    </section>
  );
